...
async def main():
    ...
    config_file = "c:/Users/admin/Documents/OmniMCP/test_config.json"
    backup_config_file = "c:/Users/admin/Documents/OmniMCP/test_config.json.bak"
    ...
    cmd = [sys.executable, "c:/Users/admin/Documents/OmniMCP/router.py", "--config", config_file]
    ...
    try:
        ...
        call_a = {
            ...
            "id": 3,
            "method": "tools/call",
            "params": {"name": "mock-alpha__slow_add", "arguments": {"a": 5, "b": 5, "delay": 2}}
        }
        call_b = {
            "jsonrpc": "2.0",
            "id": 4,
            "method": "tools/call",
            "params": {"name": "mock-beta__slow_add", "arguments": {"a": 50, "b": 50, "delay": 2}}
        }
        ...
        assert elapsed < 3.5, "L'exécution parallèle n'est pas asynchrone non-bloquante !"
        ...
        call_slow = {
            ...
            "id": 5,
            "method": "tools/call",
            "params": {"name": "mock-alpha__slow_add", "arguments": {"a": 1, "b": 1, "delay": 6}}
        }
        ...
        assert len(perf_warnings_captured) > 0, "Aucun avertissement de performance (PERF_WARNING) n'a été capturé !"
        print(f"[TEST] Succès : Warning de performance intercepté : {perf_warnings_captured[0]}")
        ...
        print("[TEST] Attente de la reconnexion automatique du serveur crashé (3.5 secondes)...")
        ...
        call_greet_reconnect = {
            ...
            "id": 7,
            "method": "tools/call",
            "params": {"name": "mock-alpha__greet", "arguments": {"name": "Auto-Healing"}}
        }
        ...
        assert "result" in res_greet and not res_greet.get("error"), "Échec de reconnexion automatique !"
        print("[TEST] Succès : Auto-Healing opérationnel, le serveur s'est reconnecté de lui-même.")
        ...
        single_server_config = {
            "mcpServers": {
                "mock-alpha": {
                    ...
                    "args": [
                        "c:/Users/admin/Documents/OmniMCP/mock_server.py",
                        "alpha"
                    ]
                }
            }
        ...
        while time.time() - start_wait < 5.0:
            try:
                ...
                notif = await asyncio.wait_for(received_notifications.get(), timeout=1.0)
        ...
        assert notification_received, "Aucune notification 'list_changed' n'a été reçue lors du Hot-Reload !"
        print("[TEST] Succès : Notification de changement de liste d'outils bien reçue par le client.")

        # Vérification finale de la liste d'outils (mock-beta doit avoir disparu)
        list_req_after = {"jsonrpc": "2.0", "id": 8, "method": "tools/list", "params": {}}
        ...
        print("[TEST] Succès : Le serveur 'mock-beta' a été déchargé à chaud avec succès.")
