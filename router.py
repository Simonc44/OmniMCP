...
class SubServerManager:
    ...
    async def _lifecycle_loop(self):
        ...
        while not self.shutdown_event.is_set():
            ...
            try:
                logger.info(f"[{self.name}] Tentative de connexion (tentative {attempt+1}/{self.max_attempts})...")
                ...
                logger.info(f"[{self.name}] Sous-serveur connecté et initialisé avec succès.")
                ...
                logger.error(f"[{self.name}] Erreur détectée dans le sous-serveur : {e}", exc_info=True)
                await self._cleanup()
                
                attempt += 1
                if attempt >= self.max_attempts:
                    logger.error(f"[{self.name}] Nombre maximal de tentatives ({self.max_attempts}) atteint. Arrêt des tentatives.")
                    self.status = "failed"
                    await self.on_tools_changed()
                    break
    ...
    async def call_tool(self, tool_name: str, arguments: dict, timeout: float = 5.0):
        ...
        if self.status == "connecting":
            logger.info(f"[{self.name}] Serveur en reconnexion. Attente de grâce ({timeout}s) pour '{tool_name}'...")
            try:
                await asyncio.wait_for(self.connect_event.wait(), timeout=timeout)
            except asyncio.TimeoutError:
                raise TimeoutError(f"Le sous-serveur '{self.name}' n'a pas pu se reconnecter dans le délai imparti.")
                
        if self.status != "connected" or not self.session:
            raise RuntimeError(f"Le sous-serveur '{self.name}' est hors-ligne (Statut: {self.status}).")
...
class HookSystem:
    ...
    async def apply_request_hooks(self, server_name: str, tool_name: str, arguments: dict) -> dict:
        current_args = arguments
        for hook in self.request_hooks:
            current_args = await hook(server_name, tool_name, current_args)
        return current_args

    async def apply_response_hooks(self, server_name: str, tool_name: str, result: types.CallToolResult) -> types.CallToolResult:
...
class RouterServer(Server):
    ...
    async def run(self, read_stream, write_stream, initialization_options, raise_exceptions=False, stateless=False):
    ...
    async def notify_tool_list_changed(self):
        ...
        for session in list(self.active_sessions):
            try:
                logger.info("Notification du client principal : changement dans la liste des outils.")
                await session.send_tool_list_changed()
            except Exception as e:
                logger.error(f"Erreur lors de l'envoi de la notification de modification d'outils : {e}")
...
class RouterGateway:
    ...
    def _register_default_hooks(self):
        ...
        async def truncate_large_responses(server_name: str, tool_name: str, result: types.CallToolResult) -> types.CallToolResult:
            ...
            for block in result.content:
                if isinstance(block, types.TextContent):
                    if len(block.text) > max_chars:
                        ...
                        modified_content.append(types.TextContent(type="text", text=truncated_text))
                        continue
    ...
    def _setup_handlers(self):
        ...
        async def handle_list_tools():
            ...
            async with self.update_lock:
                exposed_tools = []
                for exposed_name, (manager, original_name, orig_tool) in self.tool_to_server.items():
        ...
        async def handle_call_tool(name: str, arguments: dict):
            ...
            try:
                ...
                err_msg = f"Erreur de validation des arguments pour '{name}' : {e.message}"
                logger.error(err_msg)
                return types.CallToolResult(
                    isError=True,
                    content=[types.TextContent(type="text", text=err_msg)]
            ...
            try:
                mutated_args = await self.hook_system.apply_request_hooks(manager.name, original_name, arguments)
            except Exception as e:
                err_msg = f"Erreur lors de l'exécution du hook de requête : {e}"
                logger.error(err_msg, exc_info=True)
                return types.CallToolResult(isError=True, content=[types.TextContent(type="text", text=err_msg)])
                
            # 3. Exécution avec Profiling des Performances
            logger.info(f"Routage de '{name}' -> sous-serveur '{manager.name}' (outil : '{original_name}')")
            start_time = time.perf_counter()
            
            try:
                result = await manager.call_tool(original_name, mutated_args)
            except Exception as e:
                err_msg = f"Erreur lors de l'exécution de l'outil sur '{manager.name}' : {e}"
                logger.error(err_msg, exc_info=True)
                return types.CallToolResult(
                    isError=True,
                    content=[types.TextContent(type="text", text=err_msg)]
            ...
            logger.info(f"[{manager.name}] '{original_name}' a répondu en {elapsed_ms:.2f}ms")
            ...
            try:
                mutated_result = await self.hook_system.apply_response_hooks(manager.name, original_name, result)
            except Exception as e:
    ...
    async def reload_config(self):
        ...
        async with self.update_lock:
            try:
                ...
                logger.error(f"Impossible de décoder le JSON lors du rechargement : {e}")
                return
    ...
    async def rebuild_routing_table(self):
        ...
        logger.info(f"Table de routage reconstruite : {len(self.tool_to_server)} outil(s) disponible(s).")
        ...
        while True:
            try:
                ...
                if mtime != self.last_config_mtime:
                    logger.info("Changement détecté dans le fichier de configuration. Rechargement à chaud...")
                    await self.reload_config()
                    self.last_config_mtime = mtime
            except asyncio.CancelledError:
                break
            except Exception as e:
                logger.error(f"Erreur dans la boucle de surveillance du fichier de configuration : {e}")
    ...
    async def shutdown(self):
        ...
        logger.info("Tous les sous-serveurs ont été fermés proprement. Arrêt du routeur.")
...
def main():
    parser = argparse.ArgumentParser(description="MCP Hub Router (Gateway) Industrielle")
    parser.add_argument(
        "--config", 
        type=str, 
        default=os.path.join(os.path.dirname(os.path.abspath(__file__)), "mcp_router_config.json"),
